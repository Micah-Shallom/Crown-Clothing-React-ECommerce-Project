...
//So now in order to fetch data from firstore we know that we need collections data both in the collectionsoverview page as well as in the collectionpage. So we have to make the call at the topmost level of both components to avoid making multiple calls at their respective components.
  ...
  render () {
    ...
    const {match, isCollectionFetching,isCollectionLoadedAlready} = this.props;
    // const {loading} = this.state;
    return  (
      <div className="shop-page">
       <Route exact path={`${match.url}`} render={(props) => <CollectionsOverviewWithSpinner isLoading={!isCollectionLoadedAlready} {...props} />} />
       <Route path={`${match.url}/:collectionId`} render={(props) => <CollectionPageWithSpinner isLoading={!isCollectionLoadedAlready} {...props} />} />
      </div>
    )
  }
}
